...
For large raster grids, you can significantly speed up the process by using a parallel backend. The `mirai` backend is highly recommended for its efficiency.

**Performance Benefits:**
- **mirai**: ~2-3x speedup over sequential execution
- **future**: ~1.5-2x speedup

To use parallel generation, simply configure the parallel backend before calling `inspire_grid()`. The function will automatically detect the backend and switch to parallel mode.

**Option 1: Using `mirai` (Recommended)**
...
**Option 2: Using `future`**

```r
library(future)
library(future.apply)

# Set up a multisession plan
...
- Additional formats (`.gdb`, `.sqlite`) - see `sf::st_drivers()` for full list
...
5. **Sequential vs parallel**: The disk-based approach supports parallel chunking using the `mirai` backend (~2-3x speedup). The `future` backend is currently supported for **raster output only**; vector output with `future` will run sequentially.
