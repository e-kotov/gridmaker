# --- Tests for inspire_generate ---

test_that("inspire_generate works with different input types", {
  # 1. Data frame
  coords_df <- data.frame(x = 4400000, y = 3500000)
  id_df <- inspire_generate(coords_df, cellsize_m = 1000)
  ...
  id_mat <- inspire_generate(coords_mat, cellsize_m = 1000)
  expect_equal(id_df, id_mat)

  # 3. sf object
  coords_sf <- st_as_sf(coords_df, coords = c("x", "y"), crs = 3035)
  id_sf <- inspire_generate(coords_sf, cellsize_m = 1000)
  expect_equal(id_df, id_sf)
})

test_that("inspire_generate produces correct long and short format IDs", {
  coords <- data.frame(x = 4447500, y = 3497500)
  cellsize <- 1000

  # Long format (default)
  expected_long <- "CRS3035RES1000mN3497000E4447000"
  gen_long <- inspire_generate(coords, cellsize_m = cellsize, llc = FALSE)
  expect_equal(gen_long, expected_long)

  # Short format
  expected_short <- "1kmN3497E4447"
  gen_short <- inspire_generate(
    coords,
    cellsize_m = cellsize,
    short = TRUE,
    llc = FALSE
  ...
  gen_long_100m <- inspire_generate(coords_100m, cellsize_m = 100, llc = FALSE)
  expect_equal(gen_long_100m, expected_long_100m)

  expected_short_100m <- "100mN34972E44471"
  gen_short_100m <- inspire_generate(
    coords_100m,
    cellsize_m = 100,
    short = TRUE,
    llc = FALSE
...
test_that("inspire_generate handles different coordinate column names", {
  ...
  expect_equal(inspire_generate(coords_xy, cellsize_m = 10000), base_id)
  expect_equal(inspire_generate(coords_XY, cellsize_m = 10000), base_id)
  expect_equal(inspire_generate(coords_pos, cellsize_m = 10000), base_id)
})

test_that("inspire_generate resolution guessing works", {
...
test_that("inspire_generate handles vectorization correctly", {
  ...
  generated <- inspire_generate(coords, cellsize_m = 100)
  expect_equal(generated, expected)
  expect_equal(length(generated), 3)
})

test_that("inspire_generate handles empty, NULL, and NA inputs gracefully", {
  # Empty data frame
  empty_df <- data.frame(x = numeric(0), y = numeric(0))
  expect_error(
    inspire_generate(empty_df, cellsize_m = 1000),
  ...
  expect_error(
    inspire_generate(empty_mat, cellsize_m = 1000),
    "Input 'coords' cannot be empty"
  )
  ...
  expect_error(
    inspire_generate(coords_na, cellsize_m = 1000),
